...
class ThresholdSignature:
    ...
    def __init__(self, group_size: int, threshold: int) -> None:
        ...
        # print("self pubkey: ", self.public_key)
    ...
    def sign_partial(self, k_i, pubkey, message: bytes) -> tuple:
        """Create ECDSA compact signature (recovery_id, r, s_i) with private key shares"""
        
        #hash message
        d = message_digest(message)
        e = hash_to_int(d)
        ...
        s_i = []
        for i in range(self.group_size):
            s_i.append(((e + r * self.shares[i]) * k_i[i]) % curve.n)
        return r, s_i, recovery_id, pubkey


    def interpole_and_prep_ver(self, r, s_i, recovery_id, pubkey):
        prefix = 27 + recovery_id + 4
        """Sign final s based on s_i - for sender part"""
        s = Polynomial.interpolate_evaluate(random.sample(ThresholdSignature.shares_to_points(s_i), self.signature_threshold), 0)
        serialized_sig = prefix.to_bytes(1, byteorder='big') + r.to_bytes(32, byteorder='big') + s.to_bytes(32, byteorder='big')
        return public_key_to_address(self.public_key, compressed=True), b64encode(serialized_sig).decode('ascii'), s

    def sign_message(self, recovery_id, r, s) -> tuple:
